...
void generateOperationsFromPairs(
    ...
    for (auto testPair : testPairs) {
        ...
        ops.push(operation{testPair.first, testPair.second, ret});
    }
}
...
void loadOperationsFromFile(std::string testfile, std::queue<operation>& ops) {
    std::ifstream infile(testfile);
    if (infile.is_open()) {
        std::string line;
        while (std::getline(infile, line)) {
            ...
            // std::endl;
            ops.push(operation{method, input, output});
        }
        infile.close();
    }
}

void testOperations(std::queue<operation>& ops) {
    SetLib sl;
